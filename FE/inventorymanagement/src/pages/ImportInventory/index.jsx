...
import { deleteImport, getListImport, getListPageImport } from "../../service/importService";
import { showDelete, showError, showSuccess } from "../../utils/alert";

function TransactionImportInventory() {
  ...
  const fetchApi = async (currentPage, searchValue = "", fromDate = "1970-01-01", toDate = null) => {
    ...
    const result = await getListPageImport(currentPage, 10, searchValue, fromDate, to);
  ...
  const handleSave = async () => {
    ...
    try {
      // await updatelistimport(payload); // gọi API backend cập nhật
  ...
  const hanldeSearch = async () => {
    ...
    const result = await getListPageImport(1, 10, searchValue, fromDate, toDate);
    setlistimport(result.content);
    setPage(result.pageNumber);
    setTotalPages(result.totalPages);
    ...
    <>
          ...
          <div className="row">
            <div className="col-12">
              <div className="page-title-box">
                <h5 className="page-title">Giao dịch nhập kho </h5>
              </div>
            </div>
          </div>
          <div className="row">
                  ...
                  <div className="row mb-2">
                    ...
                    <div className="col-xl-4">
                      <div className="text-xl-end mt-xl-0 mt-2">
                        <Link to="/inventory/imports/create" className="btn btn-success mb-2 me-2"><i className="mdi mdi-basket me-1"></i> Thêm </Link>
                      </div>
                    </div>
                  </div>
                  <Table >
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered size="lg">
        <Modal.Header closeButton>
          <Modal.Title>{isEditMode ? "Sửa thông tin phiếu nhập" : "Thông tin phiếu nhập"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
              ...
              <Col md={8}>
                {isEditMode ? (
                  <input
                    ...
                    value={selectedlistimport.supplierName || ""}
                    onChange={(e) =>
                  ...
                  <div>{selectedlistimport.supplier?.supplierName || "N/A"}</div>
                )}
              </Col>
            </Row>
          </div>
              ...
              <Col md={12}>
                <div className="table-responsive">
                  <Table>
                    ...
                    <tbody>
                      {selectedlistimport.importDetailList && selectedlistimport.importDetailList.map((item, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{item.productReponse?.productCode || ""}</td>
                          <td>{item.productReponse?.name || ""}</td>
                          <td>{item.productReponse?.unitReponse.name || ""}</td>
                          <td>{item.quantity || ""}</td>
...
export default TransactionImportInventory;